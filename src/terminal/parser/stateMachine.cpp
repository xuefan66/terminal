...
// Returns false for C0 characters and C1 [single-character] CSI.
constexpr bool isNonActionableFromGround(const wchar_t wch) noexcept
{
    // This is written in this peculiar way to get MSVC to emit optimal assembly.
...
[[maybe_unused]] static size_t findNextVTSequence(const wchar_t* data, size_t count, size_t i) noexcept
{
    auto p = data + i;
    ...
    for (const auto end = data + count; p != end && isNonActionableFromGround(*p); ++p)
    {
    }
    return p - data;
}
...
void StateMachine::ProcessString(const std::wstring_view string)
{
    ...
    while (i < string.size())
    {
        {
            _runOffset = i;
            i = findNextVTSequence(string.data(), string.size(), i);
